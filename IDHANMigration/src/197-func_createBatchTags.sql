-- Drop the table-level LOCK TABLE ... IN SHARE ROW EXCLUSIVE MODE guards from createbatchtags.
-- Those locks existed only because INSERT ... WHERE NOT EXISTS is not concurrency-safe, and they
-- serialized *all* concurrent tag creation server-wide. INSERT ... ON CONFLICT DO NOTHING is
-- race-safe under READ COMMITTED via each table's existing UNIQUE constraint
-- (tag_namespaces.namespace_text, tag_subtags.subtag_text, tags(namespace_id, subtag_id)),
-- so the locks are no longer needed and concurrent batches can run in parallel.
