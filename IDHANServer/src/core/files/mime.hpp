...
[[nodiscard]] MimeID registerMimeType( const std::string& name, DbClientPtr db );

//! Returns the mime id assocaited with a given record.
//! @throws NoFileInfo if the record has no file info
[[nodiscard]] drogon::Task< std::expected< MimeID, drogon::HttpResponsePtr > > getMimeIDFromRecord( RecordID id, DbClientPtr db );
...
[[nodiscard]] drogon::Task< std::expected< FileMimeInfo, drogon::HttpResponsePtr > > getMime( MimeID mime_id, DbClientPtr db );
