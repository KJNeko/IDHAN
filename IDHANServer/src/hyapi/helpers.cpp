...
{
	if ( auto opt = request->getOptionalParameter< std::string >( "hashes" ); opt.has_value() )
	{
	...
	if ( auto opt = request->getOptionalParameter< std::string >( "file_ids" ); opt.has_value() )
	{
		Json::Reader reader {};
		Json::Value file_ids {};
	...
	if ( auto opt = request->getOptionalParameter< std::string >( "hash" ); opt.has_value() )
	{
	...
	if ( auto opt = request->getOptionalParameter< RecordID >( "file_id" ); opt.has_value() )
	{
	...
	extractRecordIDsToJsonFromFiles( const Json::Value json, const drogon::orm::DbClientPtr db )
{
	...
	if ( !records_e.has_value() ) co_return std::unexpected( records_e.error() );
