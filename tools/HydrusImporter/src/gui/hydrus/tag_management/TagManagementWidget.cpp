...
{
    ui->setupUi(this);
    connect(m_autocompleteWatcher, &QFutureWatcher<std::vector<std::pair<idhan::TagID, std::string>>>::finished, this, &TagManagementWidget::on_autocompleteFinished);
    loadDomains();
}
...
void TagManagementWidget::loadDomains() {
    ...
    connect(watcher, &QFutureWatcher<std::vector<idhan::TagDomainInfo>>::finished, this, [this, watcher]() {
        ...
        for (const auto& domain : m_domains) {
            ...
            ui->domainTabs->addTab(domainWidget, QString("%1 (%2)").arg(QString::fromStdString(domain.m_name)).arg(domain.m_id));
